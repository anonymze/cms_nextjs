 ...
 */
export const flattenI18nEntities = <T extends { uuid: string; i18n: K }[], K extends object[]>(
	arr: T
): ({[key in keyof Omit<T[number], "i18n">]: string} & {[key in keyof K[number]]: string})[] => {
	...
	}) as ({[key in keyof Omit<T[number], "i18n">]: string} & {[key in keyof K[number]]: string})[];
};
