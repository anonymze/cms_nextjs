 ...
 */
export const flattenI18nEntities = <R extends string, T extends { uuid: string; i18n: K }[] = any[], K extends { [k: string]: string }[] = any []>(
	arr: T
	...
	}) as ({[key in keyof Omit<T[number], "i18n">]: string} & {[key in keyof T[number][R][number]]: string})[];
};
